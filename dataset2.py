...
class CoSOD_Train(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        for idx in range(num):
            ...
            co_gt = Image.open(co_gt_path).convert('L').resize((self.size, self.size))
            ori_sizes.append((img.size[1], img.size[0]))
            zip_data['img'] = img
            zip_data['co_gt'] = co_gt
            zip_data['img'], zip_data['co_gt'] = cv_random_flip(zip_data['img'], zip_data['co_gt'])
            zip_data['img'], zip_data['co_gt'] = random_rotate(zip_data['img'], zip_data['co_gt'])
            zip_data['img'] = color_enhance(zip_data['img'])
